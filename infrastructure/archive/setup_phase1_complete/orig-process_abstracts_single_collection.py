...
class DatabaseWriter(threading.Thread):
    ...
    def _initialize_db(self):
        ...
        # Build a set of existing indexes as tuples of (fields, type)
        existing_indexes = set()
        for idx in collection.indexes():
            if idx.get('fields'):
                fields_tuple = tuple(idx['fields'])
                index_type = idx.get('type', 'unknown')
                existing_indexes.add((fields_tuple, index_type))
        
        # Check and add indexes only if needed
        if (('categories[*]',), 'hash') not in existing_indexes:
            collection.add_hash_index(fields=['categories[*]'])
            logger.info("Added hash index on categories")
            
        if (('submitted_date',), 'persistent') not in existing_indexes:
            collection.add_persistent_index(fields=['submitted_date'])
            logger.info("Added persistent index on submitted_date")
            
        if (('pdf_status.state',), 'persistent') not in existing_indexes:
